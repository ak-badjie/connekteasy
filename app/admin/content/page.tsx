...
export default function AdminContentPage() {
  ...
  const addEvent = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await createEvent({ title: ev.title, type: ev.type, date: Timestamp.fromDate(new Date(ev.date)), description: ev.description });
  ...
  const addCourse = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await createCourse({ title: co.title, category: co.category, level: co.level, durationLabel: co.durationLabel });
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-5xl mx-auto space-y-6">
      <motion.div variants={fadeInUp}>
        <h1 className="text-2xl sm:text-3xl font-display font-bold text-gray-900">Events & Courses</h1>
        <p className="text-sm text-gray-500 mt-1">Published items appear on student & seeker dashboards.</p>
      </motion.div>
              ...
              <div key={e.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {e.type} · {e.date?.toMillis ? new Date(e.date.toMillis()).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short" }) : ""}
                  </p>
                </div>
            ...
            {courses.map((c) => (
              <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {c.category}{c.level ? ` · ${c.level}` : ""}{c.durationLabel ? ` · ${c.durationLabel}` : ""}
                  </p>
                </div>
