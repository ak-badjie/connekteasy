        ...
        <>
          ...
          <motion.div className="fixed top-0 right-0 z-50 h-full w-full sm:w-[520px] bg-white shadow-2xl" variants={slideInRight} initial="hidden" animate="visible" exit="exit">
            ...
            <div className="sticky top-0 bg-white border-b border-gray-100 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between z-10">
              <h2 className="font-display text-xs sm:text-sm font-semibold text-gray-500 uppercase tracking-wider">Project Details</h2>
            ...
            <div className="overflow-y-auto h-[calc(100%-57px)] sm:h-[calc(100%-73px)] px-4 sm:px-6 py-5 sm:py-6">
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
                ...
                <h1 className="font-display text-xl sm:text-2xl font-bold text-gray-900 leading-snug mb-2">{project.title}</h1>
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-5 sm:mb-6">
                  {[{ label: "Budget", value: project.budget }, { label: "Duration", value: project.duration }, { label: "Location", value: project.location }].map((stat) => (
                    <div key={stat.label} className="bg-soft-surface rounded-2xl p-4 sm:p-5 text-center">
                ...
                <div className="mb-5 sm:mb-6">
                  <h3 className="font-display text-sm sm:text-base font-semibold text-gray-900 mb-2">Description</h3>
                ...
                <div className="mb-5 sm:mb-6">
                  <h3 className="font-display text-sm sm:text-base font-semibold text-gray-900 mb-2">Category</h3>
              ...
              <motion.div className="sticky bottom-0 bg-white pt-3 sm:pt-4 pb-2 border-t border-gray-100 -mx-4 sm:-mx-6 px-4 sm:px-6 mt-auto" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                {!showProposalForm ? (
                  <>
                    ...
                    <motion.button className="w-full py-2.5 sm:py-3 mt-2 text-sm font-medium text-gray-600 bg-soft-surface rounded-xl hover:bg-gray-200 transition-colors" whileTap={{ scale: 0.97 }}>
                      Save for Later
                    </motion.button>
                    <motion.button
                      onClick={async () => {
                      ...
                      className="w-full py-2.5 sm:py-3 mt-2 text-sm font-semibold text-white bg-mustard-500 rounded-xl hover:bg-mustard-600 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                      whileTap={{ scale: 0.97 }}
                    >
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    <h3 className="font-display text-base font-semibold text-gray-900 mb-3">Send Proposal</h3>
                    <div className="mb-3">
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-3 text-sm border border-gray-200 rounded-xl bg-white text-gray-900 placeholder-gray-400 resize-none"
                      />
                    </div>
                    <div className="mb-4">
                    ...
                    <div className="flex gap-2">
                      <motion.button onClick={() => setShowProposalForm(false)} className="px-4 py-3 text-sm font-medium text-gray-600 bg-soft-surface rounded-xl hover:bg-gray-200 transition-colors" whileTap={{ scale: 0.97 }}>
                        Cancel
                      </motion.button>
                      <motion.button
